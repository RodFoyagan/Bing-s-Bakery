...
if (nameValue && phoneValue) {
  ...
  emailjs.send('service_ceb9ind', 'template_ngn2h5g', orderDetails) // Replace with your service and template IDs
      .then(function(response) {
          ...
          const cartUI = document.getElementById("cart"); // Replace with your cart element ID
          if (cartUI) {
              cartUI.innerHTML = ""; // Clear cart UI
          }

          // Optionally update the total cart count/price display to zero
          const cartTotalUI = document.getElementById("cart-total"); // Replace with your total UI ID
          if (cartTotalUI) {
              cartTotalUI.textContent = "0"; // Update total count/price to zero
          }

          // Clear any cart-related variables in your JavaScript
          cartItems = [];
      })
      .catch(function(err) {
